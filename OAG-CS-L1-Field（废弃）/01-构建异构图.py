...
def load_OAG_L1_Field_hg() -> dgl.DGLHeteroGraph:
    ...
    edge_index_dict = {
        ...
        ('paper', 'PV_journal', 'venue'): _edge_index_dict[('paper', 'PV_Journal', 'venue')],  
    ...
    edge_index_dict[('venue', 'VP_journal', 'paper')] = edge_index_dict[('paper', 'PV_journal', 'venue')][::-1]
    ...
    label = torch.zeros(hg.num_nodes('paper'), num_classes, dtype=torch.bool)
    for key in graph_dict["labels"]:
        label[key, graph_dict["labels"][key]] = True 
...
def main():
    ...
    with open(os.path.join(ROOT, 'OAG-L1-Field/OAG-L1-Field.dglhg.pkl'), 'wb') as fp:
        pickle.dump(hg, fp)
